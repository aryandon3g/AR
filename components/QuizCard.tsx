...
// --- 1. LOCAL TYPES (Updated to match your data format) ---
interface LocalQuizQuestion {
  question_en: string;
  question_hi: string;
  options_en: string[];
  options_hi: string[];
  // ✅ FIX: Added support for your specific data key 'correct_option_index'
  correct_option_index?: number | string; 
...
export const QuizCard: React.FC<QuizCardProps> = ({
  ...
  // 🔥 UNIVERSAL INDEX PARSER (Handles your format + others) 🔥
  const getCorrectIndex = (): number => {
    // ✅ Check your specific key first: 'correct_option_index'
    let raw = q.correct_option_index;
    
    // Fallback to other possible keys
    if (raw === undefined) raw = q.correctAnswerIndex;
    ...
    if (typeof raw === 'string') {
        ...
        if (clean.length === 1 && clean.toUpperCase() >= 'A' && clean.toUpperCase() <= 'Z') {
